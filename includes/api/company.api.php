<?php // Company Api file
  ...
  switch ( $_SERVER['REQUEST_METHOD'] ){
    case "GET":
      switch ( $_REQUEST['action'] ){
        case "get_form":
          ...
          ApiHelper::json_return(Company::get_form($mode));
          break;

        case "list":
          Auth::has_permission(["company_list"], $throw_exception = true );
          if ( !isset($_REQUEST["limit"]) || empty($_REQUEST["limit"]) )
            $_REQUEST["limit"] = 1000;

          ApiHelper::json_return($Company->list($_REQUEST));          
          break;
        ...
        case "list_datatable":
          ...
          ApiHelper::json_return($Company->get_datatable_list($Company->list($_REQUEST)));          
          break;
        ...
        case "html_table":
          ...
          $list = $Company->list($filters);
          $table = DataTable::create($Company->to_datatable($list));
    ...
    case "POST":
      switch ( $_REQUEST['action'] ){
        case "create":
          Auth::has_permission(["company_create"], $throw_exception = true );
          Helper::validate_form(Company::get_form("create"), $_REQUEST, $bypass_required = ["id",]);
          
          $Company->to_object($_POST);

          $new_data = $Company->create();
        ...
        case "edit":
          Auth::has_permission(["company_edit"], $throw_exception = true );
          
          Helper::validate_form(Company::get_form(), $_POST, $bypass_required = []);
         
          $company = $Company->list(["id" => $_POST['id']]);
          if ( !$Company )
            throw new NotFoundException(TranslationHelper::get_text(["code" => "record_not_found", "replace_values" => [ "id" => $_POST['id']]]));
          
          $new_data = $Company->edit($_POST);
        ...
        case "delete":
          Auth::has_permission(["company_delete"], $throw_exception = true );
          ...
          $Company->to_object($_POST);
          $new_data = $Company->delete($_POST['id']);
