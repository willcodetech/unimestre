<?php // Currency Api file
  ...
  switch ( $_SERVER['REQUEST_METHOD'] ){
    case "GET":
      switch ( $_REQUEST['action'] ){
        case "get_form":
          ...
          ApiHelper::json_return(Currency::get_form($mode));
          break;

        case "list":
          Auth::has_permission("currency_list");
          if ( !isset($_REQUEST["limit"]) || empty($_REQUEST["limit"]) )
            $_REQUEST["limit"] = 1000;

          ApiHelper::json_return($Currency->list($_REQUEST));          
          break;
        ...
        case "list_datatable":
          ...
          ApiHelper::json_return($Currency->get_datatable_list($Currency->list($_REQUEST)));          
          break;
    ...
    case "POST":
      switch ( $_REQUEST['action'] ){
        case "create":
          Auth::has_permission("currency_create");
          Helper::validate_form(Currency::get_form("create"), $_REQUEST, $bypass_required = ["id",]);
          
          $Currency->to_object($_POST);

          $new_data = $Currency->create();
        ...
        case "edit":
          Auth::has_permission("currency_edit");
          
          Helper::validate_form(Currency::get_form(), $_POST, $bypass_required = []);
         
          $currency = $Currency->list(["id" => $_POST['id']]);
          if ( !$Currency )
            throw new NotFoundException(TranslationHelper::get_text(["code" => "record_not_found", "replace_values" => [ "id" => $_POST['id']]]));
          
          $Currency->to_object($_POST);
          $new_data = $Currency->edit();
        ...
        case "delete":
          Auth::has_permission("currency_delete");
          ...
          $Currency->to_object($_POST);
          $new_data = $Currency->delete($_POST['id']);
